...
public class GotoStringEncryption {
    ...
    public static void gotoEncryption(ClassNode node) {
        UnicodeDictionary dictionary = new UnicodeDictionary(25);
        int[] key = new int[257];
        for (int i = 0; i < 257; i++) {
        ...
        for (MethodNode method : node.methods) {
            ...
            for (AbstractInsnNode instruction : method.instructions) {
                if (GOTOASMUtils.isInsnNodeString(instruction)) {
                    LdcInsnNode ldcInsnNode = (LdcInsnNode) instruction;
                    if (StringEncryptionManager.stringBlacklist.getOptions().contains((String) ldcInsnNode.cst))
                        return;
                    modifier.replace(instruction, new FieldInsnNode(Opcodes.GETSTATIC, node.name, fieldName, "[Ljava/lang/String;"), new LdcInsnNode(pos), new InsnNode(Opcodes.AALOAD));
        ...
        for (FieldNode field : node.fields) {
            if (Modifier.isStatic(field.access)) {
                ...
                if (value instanceof String) {
                    if (StringEncryptionManager.stringBlacklist.getOptions().contains(value))
                        return;
                    field.value = null;
        ...
        MethodNode decryptMethod =
                new MethodNode(Opcodes.ACC_PRIVATE | Opcodes.ACC_STATIC | Opcodes.ACC_SYNTHETIC,
                               decryptMethodName, "(Ljava/lang/String;I)V", null, null);
	    ...
	    //<editor-fold desc="Hard coded switch tables">
	    decryptMethod.visitTableSwitchInsn(0,
                255,
                // Dude that's insane
                labels[515],
                labels[3],
        ...
        for (int i = 0; i <= 255; i++) {
            ...
            decryptMethod.instructions.add(
                    GOTOASMUtils.createNumberNode(((i + 1) ^ (~offset)) ^ key[i]));
        ...
        decryptMethod.instructions.add(
                GOTOASMUtils.createNumberNode(257 ^ (~offset) ^ key[256]));
        decryptMethod.visitVarInsn(Opcodes.ISTORE, 4);
    ...
    private static class EncryptData {
        ...
        public EncryptData(String encryptedString, int pos) {
    ...
    private static class FieldData {
        ...
        public FieldData(int pos, FieldNode fieldNode) {
            this.pos = pos;
